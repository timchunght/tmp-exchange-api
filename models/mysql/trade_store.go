...
func (s *Store) AddTrades(trades []*models.Trade) error {
	...
	for _, trade := range trades {
		valueString := fmt.Sprintf("('%v', '%v', '%v', %v, %v, %v, %v, '%v', '%v',%v,%v)",
			time.Now(), trade.Id, trade.ProductId, trade.TakerOrderId, trade.MakerOrderId, trade.Price, trade.Size, trade.Side,
			trade.Time, trade.LogOffset, trade.LogSeq)
		valueStrings = append(valueStrings, valueString)
	}
	sql := fmt.Sprintf("INSERT IGNORE  INTO g_trade (created_at, id, product_id,taker_order_id,maker_order_id,"+
